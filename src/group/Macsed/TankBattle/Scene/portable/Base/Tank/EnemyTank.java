package group.Macsed.TankBattle.Scene.portable.Base.Tank;
...
public class EnemyTank { // 敌人坦克类
	...
	public void die() {
		hp--;
		if (hp == 0) {
			CoreTool.bomp(x, y); // 炸开
			this.x = 1000;
			this.y = 1000;
			Demo.mainViewController.enemy_tank_died(kind);
	...
	public void lauch() { // 发射的方法
		...
		if (bdir == 0) {
			GameBullet gameBullet = new GameBullet(x + 15, y - 10, 10, 10, edir, attack * 10, 2);
			Demo.gameDataManager.getGameBullet().add(gameBullet);
		} else if (bdir == 1) {
			GameBullet gameBullet = new GameBullet(x + 15, y + 40, 10, 10, edir, attack * 10, 2);
			Demo.gameDataManager.getGameBullet().add(gameBullet);
		} else if (bdir == 2) {
			GameBullet gameBullet = new GameBullet(x - 10, y + 15, 10, 10, edir, attack * 10, 2);
			Demo.gameDataManager.getGameBullet().add(gameBullet);
		} else if (bdir == 3) {
			GameBullet gameBullet = new GameBullet(x + 40, y + 15, 10, 10, edir, attack * 10, 2);
			Demo.gameDataManager.getGameBullet().add(gameBullet);
		}
